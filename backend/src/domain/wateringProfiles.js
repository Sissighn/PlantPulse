...
function getPlantProfile(name, type) {
  const candidates = [type, name].map(normalize).filter(Boolean);
...
function getProfileBaseInterval(name, type) {
  return getPlantProfile(name, type)?.baseInterval || DEFAULT_BASE_INTERVAL;
}

module.exports = {
  DEFAULT_BASE_INTERVAL,
  PLANT_WATERING_PROFILES,
  PROFILE_TYPES,
  getPlantProfile,
  getProfileBaseInterval,
};
